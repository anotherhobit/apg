...
void DrawTestScene2C(void)
{
  ...
  colorbuffer = (float*)sglGetColorBufferPointer();
}
...
/// NFF drawing test
float RayTraceScene(const char *scenename)
{
...
int main(int argc, char **argv)
{
  ...
  // general ray tracer code invoked when a parameter is used to execute the file
  if (argc > 1) {
    sglSetContext(_contexts[0]);
    double time = RayTraceScene(argv[1]);
    cout<<"Time:"<<time<<endl;
    char savename[256];
